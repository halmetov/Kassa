...
async def get_summary(db: AsyncSession = Depends(get_db)):
    sales_result = await db.execute(select(Sale, User.name, Branch.name).join(User).join(Branch))
    sales = []
    ...
    by_seller_query = await db.execute(
    ...
    by_branch_query = await db.execute(
