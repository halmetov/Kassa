...
async def create_product(payload: product_schema.ProductCreate, db: AsyncSession = Depends(get_db)):
...
async def update_product(product_id: int, payload: product_schema.ProductUpdate, db: AsyncSession = Depends(get_db)):
...
async def upload_photo(product_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    product = await db.get(Product, product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
